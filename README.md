...
v1.3.2 retains the v1.3.1 live-missile validation and deferred retargeting. Binary inspection showed that the verified core's `OnMissionTick` called native missile position and velocity getters before checking `AwaitingCollisionReaction`. The sidecar now quarantines wrappers in that state for the duration of the mission tick, suppresses their native reads and writes, defers registry refresh until the collision reaction resolves, and excludes them from leader/camera ownership repair. The experimental collision-queue expansion was removed after equal 32-arrow and 48-arrow results ruled out that boundary. Repeated impacts reuse managed collision positions, penetration continuations retain the fixed managed 1.25-metre exit distance, and the per-missile consumed-target list continues to prevent only that projectile from reacquiring a target it already hit.
