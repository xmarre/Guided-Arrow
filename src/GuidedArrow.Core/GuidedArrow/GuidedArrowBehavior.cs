...
public sealed class GuidedArrowBehavior : MissionView
{
	...
	{
		public MissileCollisionReaction Reaction;
	...
	public override void OnAgentShootMissile(Agent shooterAgent, EquipmentIndex weaponIndex, Vec3 position, Vec3 velocity, Mat3 orientation, bool hasRigidBody, int forcedMissileIndex)
	{
		...
		bool flag = shooterAgent == mainAgent;
		Settings instance = GlobalSettings<Settings>.Instance;
		bool flag2 = instance != null && instance.EnableAlliedArrowTakeover && _alliedTakeoverChainArmed && IsFriendlyAlliedShooter(shooterAgent, mainAgent);
		if (_state != State.Idle)
		{
			...
			{
				Log("Ignored normal missile callback while guided-shot generation " + _activeShotGeneration + " remained active.");
			}
		}
		else if (flag)
		{
	...
	private bool IsSplitSiblingAcquisitionOpen()
	{
		...
		double num = (double)(Stopwatch.GetTimestamp() - _splitSiblingAcquireStartTimestamp) / (double)Stopwatch.Frequency;
		if (double.IsNaN(num) || double.IsInfinity(num) || num < 0.0 || num >= 0.10000000149011612)
		{
			CloseSplitSiblingAcquisition("RealTimeLaunchWindowExpired");
			return false;
		}
		TrackedMissile trackedMissile = FindTrackedMissile(_missileIndex);
	...
	public override void OnMissileHit(Agent attacker, Agent victim, bool isCanceled, AttackCollisionData collisionData)
	{
		...
		if (trackedMissile != null)
		{
			try
			{
				Missile missile = trackedMissile.Missile;
		...
		{
			UpdateCrosshairVisibility();
			Log("Missile hit registered; awaiting native collision reaction.");
		}
	}
	...
	private bool TryAcquireMissiles()
	{
		...
		for (int i = 0; i < _pendingShotSeeds.Count && _trackedMissiles.Count < 64; i++)
		{
			...
			Missile val = FindCandidateForShotSeed(pendingShotSeed);
			if (val == null)
			{
				continue;
			}
	...
	private void AcquireSplitShotSiblings()
	{
		...
		try
		{
			foreach (Missile item in (List<Missile>)(object)mission.MissilesList)
			{
	...
	private bool ValidateSyntheticMissileEntity(Missile missile, string source)
	{
	...
	private TrackedMissile CreateTrackedMissileFromSpawn(Missile missile, TrackedMissile source, Vec3 launchPosition, Vec3 launchVelocity, bool continuation)
	{
	...
	private Missile FindCandidateForShotSeed(PendingShotSeed seed)
	{
		//IL_00f6: Unknown result type (might be due to invalid IL or missing references)
	...
	private bool IsAutoguidanceSegmentObstructed(Vec3 start, Vec3 end, TrackedMissile tracked, out Vec3 hitPoint, bool stopBeforeTarget)
	{
		...
		try
		{
			...
			{
				Missile missile = tracked.Missile;
	...
	private bool TryRefreshTrackedMissileHandle(TrackedMissile tracked, out Missile liveMissile)
	{
		//IL_007f: Unknown result type (might be due to invalid IL or missing references)
		//IL_0084: Unknown result type (might be due to invalid IL or missing references)
		liveMissile = null;
		...
		try
		{
			foreach (Missile item2 in (List<Missile>)(object)((MissionBehavior)this).Mission.MissilesList)
			{
	...
	public override void OnMissileCollisionReaction(MissileCollisionReaction collisionReaction, Agent attackerAgent, Agent attachedAgent, sbyte attachedBoneIndex)
	{
		//IL_0033: Unknown result type (might be due to invalid IL or missing references)
		//IL_0023: Unknown result type (might be due to invalid IL or missing references)
	...
	private void QueueEarlyCollisionReaction(MissileCollisionReaction reaction, Agent attacker, Agent attachedAgent)
	{
		//IL_0029: Unknown result type (might be due to invalid IL or missing references)
		//IL_002a: Unknown result type (might be due to invalid IL or missing references)
	...
	private void TryConsumeEarlyCollisionReaction(int missileIndex)
	{
		...
		for (int j = 0; j < _earlyCollisionReactions.Count; j++)
		{
			...
			{
				MissileCollisionReaction reaction = earlyCollisionReaction.Reaction;
	...
	private unsafe void ResolveCollisionReaction(int missileIndex, MissileCollisionReaction reaction, string source)
	{
		//IL_0075: Unknown result type (might be due to invalid IL or missing references)
		...
		else if (flag && HasRemainingAgentPenetration(trackedMissile))
		{
			...
			Log("Queued controlled penetration continuation after native terminal agent collision " + ((object)(*(MissileCollisionReaction*)(&reaction))/*cast due to constrained. prefix*/).ToString() + "; native missile creation is deferred until the next mission tick.");
		}
		else
		{
			...
			Log("Missile #" + missileIndex + " terminated by native collision reaction " + ((object)(*(MissileCollisionReaction*)(&reaction))/*cast due to constrained. prefix*/).ToString() + " via " + source + ".");
			if (_state == State.Guiding && _trackedMissiles.Count == 0)
			{
				HandleGuidedSwarmTerminal("AllMissilesCollisionTerminal/" + ((object)(*(MissileCollisionReaction*)(&reaction))/*cast due to constrained. prefix*/).ToString());
			}
		}
	}
	...
	private void ProcessDeferredNativeMissileWork()
	{
		...
		for (int num = _pendingNativeMissileRemovals.Count - 1; num >= 0; num--)
		{
			...
			{
				Missile val = FindExactLiveMissileForDeferredRemoval(pendingNativeMissileRemoval);
				if (val != null)
				{
					try
					{
	...
	private Missile FindExactLiveMissileForDeferredRemoval(PendingNativeMissileRemoval pending)
	{
	...
	private bool TrySpawnPenetrationContinuation(TrackedMissile source, PendingCollisionContext collisionContext, out TrackedMissile continuation)
	{
		...
		//IL_01ce: Unknown result type (might be due to invalid IL or missing references)
		continuation = null;
		if (source == null || collisionContext == null || ((MissionBehavior)this).Mission == null || _activeShotShooter == null || !source.SpawnWeaponValid || source.ResolvedLaunchData == null)
		{
			return false;
		}
		...
		Vec3 val2 = collisionContext.ImpactPosition + val * 0.42f;
		if (!IsFinite(val2))
		{
			return false;
		}
		Mat3 val3 = (source.SpawnOrientationValid ? source.SpawnOrientation : Mat3.Identity);
		float num2 = ((source.SpawnBaseSpeed > 1E-06f) ? source.SpawnBaseSpeed : num);
		Missile missile;
		try
		{
			using IDisposable disposable = MissileDamageBridge.OverrideNextSyntheticMissile(((MissionBehavior)this).Mission, _activeShotShooter, source.ResolvedLaunchData);
			if (disposable == null)
			{
				return false;
			}
			missile = ((MissionBehavior)this).Mission.AddCustomMissile(_activeShotShooter, source.SpawnWeapon, val2, val, val3, num2, num, source.SpawnHasRigidBody, (MissionObject)null, -1);
		}
		catch (Exception ex)
		{
		...
		continuation = CreateTrackedMissileFromSpawn(missile, source, val2, val * num, continuation: true);
		...
		for (int num2 = _pendingCollisionContexts.Count - 1; num2 >= 0; num2--)
		{
			...
			else
			{
				...
				if (double.IsNaN(num3) || double.IsInfinity(num3) || !(num3 >= 0.0) || !(num3 < 0.05000000074505806))
				{
					...
					_pendingCollisionContexts.RemoveAt(num2);
					ResolveCollisionReaction(missileIndex, (MissileCollisionReaction)(-1), "CollisionReactionTimeout");
					if (_state != State.Guiding)
					{
						return;
					}
	...
	private void ApplyFullDetailVisual(TrackedMissile tracked)
	{
		...
		try
		{
			...
			Missile missile2 = tracked.Missile;
			object obj;
			if (missile2 == null)
			{
				obj = null;
	...
	private void TrackHitVictim(Agent victim, int missileIndex, int collisionBoneIndex, GameEntity arrowEntity, Vec3 impactDirection, Vec3 impactPosition)
	{
		...
		try
		{
			victim.OnAgentHealthChanged += new OnAgentHealthChangedDelegate(OnHitVictimHealthChanged);
	...
	private void ResetTrackedVictim()
	{
		...
		for (int i = 0; i < _pendingHitVictims.Count; i++)
		{
			...
			try
			{
				if (val != null)
				{
					val.OnAgentHealthChanged -= new OnAgentHealthChangedDelegate(OnHitVictimHealthChanged);
				}
			}
			catch
			{
	...
	private void SetOwnTimeSpeed(float requested)
	{
		...
		try
		{
			((MissionBehavior)this).Mission.AddTimeSpeedRequest(new TimeSpeedRequest(num, 1195463255));
	...
	private void SetCinematicTimeSpeed(float requested)
	{
		...
		try
		{
			((MissionBehavior)this).Mission.AddTimeSpeedRequest(new TimeSpeedRequest(num, 1195459401));
	...
	private static bool IsArrowOrBolt(Missile missile)
	{
		//IL_0007: Unknown result type (might be due to invalid IL or missing references)
		if (missile == null)
		{
